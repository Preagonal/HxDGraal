...
class GByte5aConverter : public TExternalDataTypeConverter
{
public:
    static TExternalDataTypeConverter* Create()
    {
        return new GByte5aConverter();
    }

public:
    GByte5aConverter()
    {
        FTypeName = L"Graal - GBYTE5a";
...
TBytesToStrError GByte5aConverter::BytesToStr(uint8_t* Bytes, int ByteCount, TIntegerDisplayOption IntegerDisplayOption, int& ConvertedByteCount, std::wstring& ConvertedStr)
{
    if (ByteCount >= 5)
    {
...
TStrToBytesError GByte5aConverter::StrToBytes(std::wstring Str, TIntegerDisplayOption IntegerDisplayOption, std::vector<uint8_t>& ConvertedBytes)
{
    /*
